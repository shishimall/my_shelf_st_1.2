# my_shelf_st
# 📦 JANCodeLookup + Google Sheets連携 + OpenAI OCR（gpt-4o-mini対応・Cloud安定版）
# ☁️ Streamlit Cloud / ローカル 両対応（OpenAIクラス初期化バグ回避）

import streamlit as st
st.set_page_config(page_title="my_shelf v1.208", layout="wide")
st.title("📦 my_shelf v1.208（Cloud安定動作版）")
...
import openai  # ✅ ← 旧方式を採用（Cloudバグ回避）
...
openai.api_key = api_key  # ✅ ← この方式がCloudでも安定
...
def analyze_code_with_openai(image_bytes: bytes, allow_alnum=False):
    try:
        ...
        # ✅ Cloud安定動作のためChatCompletion直呼び出し
...
st.caption("© 2025 my_shelf v1.208 — Cloud安定対応版（OpenAIクラス不使用）")
